...
fs.readFile('Day 13/solution.txt', 'utf-8', (err, data) => {
    if (err) throw err;
    ...
    // while (complete == false) {
    let input = [0];
    let boardOutput = intcode(nvarr, [0], [0,0,0,0,0,0,0]);
    for (let i = 0; i < boardOutput.length; i++) {
        switch (i%3) {
            case 0:
                currX = boardOutput[i];
                break
            ...
            case 2:
                if (currX == -1 && currY == 0) {
                    console.log("New score: " + boardOutput[i]);
                } else {
                    board[currY][currX] = boardOutput[i];
                }
        }
    }

...
fs.readFile('Day 19/solution.txt', 'utf-8', (err, data) => {
    if (err) throw err;
    ...
    const findSquare = () => {
        ...
        let j = 0;
        while (true) {
            while (intcode({...nvarr}, [0], [i, j]) == 0) {
                j++;
            }
            if (intcode({...nvarr}, [0], [i - 99, j + 99]) == 1) {
                console.log("Success! " + (i - 99) + "|" + j);
                return;
            }
            while (intcode({...nvarr}, [0], [i, j]) == 1) {
                i++;
            }
        }
    }
    for (let i = 0; i < 10000; i++) {
        ...
        for (j; j < i + 110; j++) {
            // console.log(i + "|" + j);
            if (intcode({...nvarr}, [0], [i, j]) == 1) {
                total++; 
                board[i][j] = 1;
                // if (i > 100 && j > 0) {
                //     if (intcode({...nvarr}, [0], [i + 1, j] === 1) && intcode({...nvarr}, [0], [i, j - 1]) === 1) {
                //         if (intcode({...nvarr}, [0], [i - 99, j + 99]) === 1) {
